...
Beachten Sie, dass sich um K **Raumladungen** ausbilden. Der Ladevorgang eines Kondensators bei Anliegen einer Gleichspannung 
$$
\begin{equation}
...
Falls Sie durch Gleichung **(3)** eine hinreichend gute Beschreibung der Daten erzielen können erhalten Sie $I_{\mathrm{Ph}}(t)$ aus der Ableitung von Gleichung **(3)** nach der Zeit:
$$
\begin{equation*}
I_{\mathrm{Ph}} = \frac{3}{2}\,C\,\bigl(\kappa\left(t-t_{0}\right)\bigr)^{1/2}.
\end{equation*}
$$
Für die Messung am besten geeignet ist der quasi-lineare Anstieg von $U(t)$ für $t\approx t_{0}$, so lange die sich aufbauende Gegenspannung noch klein ist. Andernfalls bestimmen Sie $I_{\mathrm{Ph}}$ aus der Steigung der Tangente für $t\approx t_{0}$, wie in **Abbildung 3** gezeigt.
